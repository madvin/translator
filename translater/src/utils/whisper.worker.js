...
class MyTranscriptionPipeline {
    ...
    static async getInstance(callback) {
        if (this.instance === null) {
            this.instance = await pipeline(this.task, this.model, { progress_callback: callback });
        }
...
async function transcribe(audio) {
    ...
    let modelPipeline;
    try {
        modelPipeline = await MyTranscriptionPipeline.getInstance(load_model_callback);
    } catch (error) {
    ...
    const generationTracker = new GenerationTracker(modelPipeline, stride_length_s);
